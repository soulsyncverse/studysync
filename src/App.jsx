...
function isCustomExamId(key,customExamIds=[]){return customExamIds.includes(key);}
    ...
    (async()=>{
      try{
       ...
       const [datesSnap,tipsSnap,selectionSnap,legacySnap,customSubjSnap,examSubjectsSnap,customExamsSnap]=await Promise.all([
        ...
        const rawKey=selection?.key||"";
        const key=EXAMS[rawKey]?rawKey:(customExamIds.includes(rawKey)?rawKey:DEFAULT_EXAM_KEY);
        const mode=EXAMS[key]?.modes?.[selection?.mode]?selection.mode:(customExamIds.includes(key)?"Exam":defaultModeFor(key,customExamIds));
