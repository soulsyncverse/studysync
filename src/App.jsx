...
// ── TEMPORARY DIAGNOSTIC — viewport height measurement, remove after investigation ──
...
useEffect(()=>{
  const measure=(label)=>{
    ...
    console.log("[viewport-diag]",label,data);
    setDiagLog(prev=>({...(prev||{}),[label]:data}));
  };
  measure("initial");
  const t1=setTimeout(()=>measure("100ms"),100);
...
return () => {active=false;unsub();};
  ...
  return(<div ref={rootRef} style={{minHeight:"100vh",background:"yellow",fontFamily:"'DM Sans','Segoe UI',sans-serif",color:t.text,transition:"background .3s"}}>
    {/* ── TEMPORARY DIAGNOSTIC PANEL — remove after investigation ── */}
    {diagLog&&<div style={{position:"fixed",top:8,left:8,zIndex:99999,background:"rgba(0,0,0,0.85)",color:"#0f0",fontFamily:"monospace",fontSize:9,padding:"6px 8px",borderRadius:6,maxWidth:260,pointerEvents:"none",whiteSpace:"pre"}}>
      {Object.entries(diagLog).map(([label,d])=>
        `${label}: ih=${d.innerHeight} oh=${d.outerHeight} sh=${d.screenHeight} vv=${d.visualViewport} dch=${d.docClientHeight} bch=${d.bodyClientHeight} root=${d.rootHeight}`
      ).join("\n")}
    </div>}
    ...
    {/* Nav */}
    <div className="ss-bottom-nav" style={{position:"fixed",bottom:0,left:8,right:8,background:"red",backdropFilter:"blur(20px)",border:`1px solid ${t.border}`,borderRadius:18,zIndex:100,padding:"7px 0 10px"}}>
      {/* Pro row */}
